...
const Certificates: React.FC<ProfProps> = () => {
  ...
  return (
    <MaxWidthWrapper className="mb-12 mt-9 sm:mt-12 flex flex-col items-center justify-center text-center">
      <AnimatedGradientText>Certificates</AnimatedGradientText>

      {Object.keys(userData as Record<string, any>).map(
        (category: string, index: number) => (
          <div key={index} className="my-8">
            <h2 className="text-2xl font-semibold mb-4">{category}</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {Array.isArray((userData as Record<string, any>)[category]) &&
                (userData as Record<string, any>)[category].map(
                  (
                    certificate: { event_name: string; img: string },
                    i: number,
                  ) => (
                    <div
                      key={i}
